...
> ℹ️ **개발 현황.** 1~3단계 핵심 기능(날씨·탐방로 → 컨디션 점수·장비 추천·인증 → 지도·PWA)에 더해 품질·성능·운영(Phase 6: 성능·접근성·계측/모니터링·CI·배포)과 상세 화면 정보 확장까지 완료했습니다. 이어 **개인화·콘텐츠 확장(Phase 7)**도 완료해 100대명산 목록(`/top100`), 방문완료 기록(`/visited`), 마이페이지 개인화 허브(`/mypage`)와 프로필 편집을 제공하고, 홈을 오늘 컨디션 신호 중심으로 재구성했습니다. 단계별 상세·진행 상황은 [`docs/PRD.md`](docs/PRD.md), [`docs/ROADMAP.md`](docs/ROADMAP.md)를 참고하세요.
...
| **개인화·콘텐츠** | 100대명산 목록, 방문완료 기록, 마이페이지·프로필, 컨디션 중심 홈 | ✅ 완료 |
...
```
app/
  (main)/            # 앱 본체 — 홈/검색, 산 상세(mountains/[id]), 즐겨찾기, 100대명산(top100),
                     #   방문완료(visited), 마이페이지(mypage/·mypage/profile) (공통 헤더 레이아웃)
  auth/              # 로그인·회원가입·비밀번호 재설정·이메일 확인
  api/               # 서버 라우트 핸들러 (검색·날씨·대기·자외선·컨디션·즐겨찾기·방문완료·계측 등)
...
lib/
  ...
  trails/            # 탐방로 CSV 파싱·계절 통제 판정·코스 요약 (순수 로직)
  geo/               # 위경도→기상청 격자 변환·일출/일몰 계산
  search/            # 산 검색·한글 초성 매칭 (순수 로직)
  profile/           # 프로필 프리셋 옵션(아이콘·경력·지역)
