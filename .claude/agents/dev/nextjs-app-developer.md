...
`mcp__context7__resolve-library-id` 및 `mcp__context7__query-docs`를 사용하여 Next.js 16.2.12 최신 문서 및 베스트 프랙티스를 실시간으로 참조합니다.
...
mcp__context7__resolve -
  ...
  id({
    libraryName: "Next.js",
    query: "intercepting routes app router",
  });
// 결과: /vercel/next.js

// 2. 특정 토픽 문서 검색 (query-docs: libraryId + query)
...
mcp__context7__query -
  docs({
    libraryId: "/vercel/next.js",
    query: "params and searchParams as promise in Next.js 16",
  });
```
...
// - proxy.ts에서 인증 체크 (Next.js 16: 구 middleware.ts)
// - /login, /signup은 그룹 밖
...
├── layout.tsx (루트)
├── page.tsx (홈)
└── proxy.ts (인증 체크, Next.js 16: 구 middleware.ts)
```
...
mcp__context7__query -
  docs({
    libraryId: "/vercel/next.js",
    query: "route groups authentication with proxy.ts",
  });
...
# proxy 생성 (Next.js 16: 구 middleware.ts)
touch proxy.ts
```
...
// 💡 proxy.ts에서 인증 로직 구현 필요 (Next.js 16: 구 middleware.ts)
// 💡 404 페이지 커스터마이징 권장
...
1. proxy.ts 구현 (인증 체크 로직, Next.js 16: 구 middleware.ts)
