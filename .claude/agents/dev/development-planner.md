...
> **⚠️ 이 저장소의 로드맵은 허브+스포크 구조입니다(2026-08-10 분리).** 신규 로드맵을 처음 생성할 때가 아니라 **기존 로드맵을 유지·갱신**할 때는 다음을 지키세요.
>
> - `docs/ROADMAP.md`(허브) = 목차·워크플로우·상태 표기 규칙·Phase↔PRD 매핑·**Phase 인덱스 표**·Task 의존성 요약·리스크. 개별 Task 상세는 두지 않습니다.
> - `docs/roadmap/phase-*.md`(스포크) = Task 상세(수락 기준·구현 단계·테스트·산출물). 새 Task 추가·Task 상태 변경은 **해당 Phase 스포크 파일**에서 수행하고, Phase 단위 상태만 허브 인덱스 표에 반영합니다.
> - 스포크 분할: `phase-0-2-foundation.md`(001~~013) · `phase-3-4-mvp.md`(014~~027) · `phase-5-7-quality-personalization.md`(028~~038) · `phase-8-11-expansion.md`(039~~050). 새 Phase 는 성격이 맞는 기존 스포크에 추가하거나 새 스포크를 만들고 허브 인덱스에 링크합니다.
