...
- **Task 042: 산 추천 (지역·고도·난이도 필터)**
  ...
  - **관련 파일**: 신규 `lib/data/recommend.ts`(필터 쿼리·`'use cache'`), 신규 `lib/trails/summary.ts` 재사용(난이도 파생), 신규 `components/mountain-recommend.tsx`, `app/(main)/page.tsx` 또는 신규 `/discover` 라우트(+`proxy.ts` 공개 경로 편입 시)
  - **수락 기준**:
    - [ ] 지역·고도대·난이도 조합으로 산 목록을 필터링할 수 있다(클라이언트 필터, 데이터 재요청 없음).
    - [ ] 100대명산 목록처럼 컨디션 점수는 카드별 스트리밍(선택) 또는 미표시로 외부 호출 폭증을 방지한다.
    - [ ] 결과 없음 빈 상태·색상 단독 금지·키보드 접근성(`aria-pressed`)을 준수한다.
  - **구현 단계**:
    - [ ] 추천 후보 데이터 계층(지역·고도·난이도 파생 포함) `'use cache'`로 구성.
    - [ ] 필터 UI(칩/셀렉트) + 정렬(고도·난이도) 클라이언트 컴포넌트.
    - [ ] 진입 지점(홈 섹션 또는 전용 라우트) 배치, 로딩 스켈레톤·`LoadingBar` 관례 재사용.
  - **테스트 체크리스트 (Playwright MCP)**:
    - [ ] 지역·고도·난이도 필터 조합 결과 정확성(경계값 포함) 확인.
    - [ ] 추천 카드 → 상세 이동·실날씨 연동, 360/768px 오버플로 0, 콘솔 에러 0, `typecheck`·`lint`·`build` 통과.
