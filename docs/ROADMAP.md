...
- **Task 001: 외부 데이터 소스 실현성 검증(PoC)** - 진행중 (데이터 소스 확보 / 잔여: 기상·대기·자외선 API 키)
  ...
  - ✅ 🔶#6 자외선 API 확정 — **기상청 생활기상지수 조회서비스(4.0) `getUVIdxV4`**, 지역코드(`areaNo`) 매핑. 활용신청 승인 완료, 게이트웨이 활성화 대기
  - ⏳ 공공데이터포털 API 키 발급(기상청 단기예보·에어코리아·생활기상지수) 및 일일 호출 한도 기록 — **사용자(archy712) 액션 대기** (탐방로는 정적 CSV라 키 불필요)
  - **산출물**: ✅ `docs/decisions/001-data-sources.md`, ✅ 등산로 GeoJSON 변환 PoC(`scratchpad/sutong_course.geojson`) / (대기) 기상·대기·자외선 소스별 샘플 응답 JSON
...
- **Task 022: 자외선 지수 연동 구현**
  - 기상청 생활기상지수(4.0) 자외선 클라이언트 구현 — `GET /1360000/LivingWthrIdxServiceV4/getUVIdxV4` (`areaNo` 행정구역코드, `time` YYYYMMDDHH 06·18시 발표), 시간대별 UV 값 파싱
