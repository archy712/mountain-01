...
| 지도        | 카카오맵 JS SDK (3단계)                                                               |
| 외부 API    | 기상청 단기예보, 탐방로 개방정보, 에어코리아 대기오염정보, 기상청 생활기상지수        |
...
- **Task 001: 외부 데이터 소스 실현성 검증(PoC)** - 진행중 (정책 확정 / PoC 대기)
  - ✅ 🔶#4 **탐방로 개방정보 소스 확정** — **국립공원공단 API 단일 소스 + "정보 없음" 폴백 정식 스펙화** 결정. 커버리지 공백 측정은 키 발급 후 PoC로 검증
  - ✅ 🔶#15 **등산로 GeoJSON 전략 확정** — **PoC 확인 후 유연 대응**(확보 시 폴리라인 오버레이 / 미확보 시 마커+탐방로 목록 축소안) 결정
  ...
  - ✅ 🔶#6 자외선 API 확정 — **기상청 생활기상지수(자외선지수)**, 지역코드 매핑
  - ⏳ 공공데이터포털 API 키 발급 및 일일 호출 한도 기록 — **사용자(archy712) 액션 대기, 이후 PoC 검증 진행**
  - **산출물**: ✅ `docs/decisions/001-data-sources.md` / (대기) 소스별 샘플 응답 JSON, 커버리지 검증표
...
- **Task 017: 탐방로 개방정보 연동 및 동기화 구현**
  - 탐방로 개방정보 API 클라이언트 구현 (Task 001 확정 소스 기준) — 산명·탐방로명·개방상태·통제사유·통제기간 파싱
  - 외부 산/탐방로명 → `mountains`/`trails` 매핑 로직 구현 (명칭 정규화, 매칭 실패 시 미매핑 로그)
  - `trails` 동기화 전략 구현 — 주기적 갱신 시 `status`/`closed_reason` upsert, 서비스 롤 쓰기
  - Route Handler `GET /api/trails?mountainId=` 구현 — 캐시 TTL 1～6시간 적용
  - **커버리지 공백 폴백** — 데이터 미보유 산은 상태 "정보 없음"으로 명시 표기(오해 방지 문구 포함)
  - **테스트 체크리스트**: Playwright MCP로 개방/통제/부분통제/정보없음 4개 케이스 배지 렌더링 확인, API 실패 시 날씨는 정상 표시되는지 확인
  - **산출물**: `app/api/trails/route.ts`, `lib/api/trail-status.ts`, `lib/api/mountain-name-matcher.ts`
  - **의존성**: Task 001, Task 014, Task 015
...
- **Task 022: 자외선 지수 연동 구현**
  - 기상청 생활기상지수(자외선) 클라이언트 구현 — 지역코드 매핑 및 시간대별 `uvIndex` 파싱
...
> **Task 001의 🔶#15 결론에 따라 Task 029의 범위가 결정된다.** GeoJSON 미확보 시 폴리라인 오버레이를 제외하고 산 위치 마커 + 탐방로 목록으로 축소한다.
...
- **Task 029: 등산로 GeoJSON 적재 및 오버레이 구현**
  - `trails.path_geojson` 데이터 적재 파이프라인 구축 (좌표 검증·단순화 포함), 스키마 변경 시 `database.types.ts` 재생성
  - `TrailOverlay` 구현 — 카카오맵 폴리라인으로 코스 렌더링, 좌표 수 과다 시 간소화
  - **통제 구간 색상 구분** — `trails.status`별 폴리라인 색상 및 `MapLegend` 연동 (색상 단독 구분 금지: 범례 텍스트 병기)
  - GeoJSON 미보유 탐방로는 목록으로만 표시하는 축소 폴백
  - **테스트 체크리스트**: Playwright MCP로 폴리라인 렌더링, 통제 구간 색상 구분, GeoJSON 없는 산의 폴백 동작 확인
  - **산출물**: `components/trail-overlay.tsx`, `components/map-legend.tsx`, `supabase/migrations/*_trails_geojson.sql`
  - **의존성**: Task 028
...
| 리스크                                            | 영향                            | 대응                                                                                            |
| ------------------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------- |
| 🔶#4 탐방로 데이터 커버리지 부족 (국립공원 외 산) | 1단계 **Must** 기능 미충족      | Task 001에서 커버리지 측정 → 대체 소스 확보 또는 "정보 없음" 명시 폴백을 정식 스펙으로 승격     |
| 🔶#15 등산로 GeoJSON 미확보                       | 3단계 핵심 기능 소멸            | Task 001에서 조기 판정 → 미확보 시 Task 029를 마커+목록 축소안으로 대체, Phase 5 일정 재산정    |
| 공공 API 호출 쿼터/장애                           | 서비스 신뢰성 목표(95～98%) 미달 | Task 015의 서버 캐싱 일원화 + stale 응답 폴백("N분 전 기준"), Task 034에서 성공률 상시 모니터링 |
