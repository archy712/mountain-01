...
- **⏸ Task 044: 실시간 탐방로 통제·공지 (정적 스냅샷 보완)** — **보류(개인 접근 소스 부재, 2026-08-10)**
  - **개요**: 정적 2023 CSV 스냅샷의 한계인 실시간 돌발 통제를 국립공원공단 실시간 통제/공지로 보완하려 했으나, **개인이 합법·안정적으로 쓸 수 있는 소스가 없어 보류**한다. 정적 CSV + 계절판정(`seasonal-closure.ts`) 베이스라인 유지.
  - **보류 사유(1단계 검증 결과, 결정 001 "실시간 탐방로 통제(Task 044)" 절)**:
    - 실시간 통제 데이터는 KNPS 웹(`acsCtrDtl.do`)에 존재하나, 대응 **data.go.kr OpenAPI 는 공공기관 전용**이라 개인 개발계정 활용신청 불가.
    - KNPS `robots.txt` 가 `/front/portal/safe/` 를 차단(`Disallow: /`)해 웹 스크래핑도 정책 위반.
    - 산불조심기간 등 예측 가능 통제는 이미 계절판정으로 결정적 반영 → 핵심 공백 제한적.
  - **재개 조건**: 국립공원공단이 개인 대상 실시간 통제/공지 API 를 개방하거나, `robots.txt` 허용 하의 공식 피드가 생기면 재검토. 그 전까지는 최신 CSV 재적재로 정확도만 주기 보정.
  - **원안(재개 시 참고) 수락 기준/구현 단계**:
    - [ ] 실시간 통제/공지가 있으면 상세 탐방로 상태에 우선 반영("실시간" 표기), 실패 시 정적+계절 폴백, 중복 dedupe, 국립공원 외 "정보 없음".
    - [ ] 실시간 응답 정규화 + 코스 식별키 병합(실시간>정적) → `/api/trails` 짧은 TTL 캐싱 + api_logs 계측 → 사유·기간·출처 표기 UI.
